...
class Rice(gym.Env):
    ...
    def __init__(
            ...
            max_altruism=0,
            final_altruism=0,
            **kwargs,  # Ignore args for other environments
    ):
        """TODO : init docstring"""
        ...
        self.ccf = CCF(self, kwargs.get('ccf', {}))
        ...
        # JH: altruism options:
        self.altruism = self.initial_altruism = initial_altruism
        self.max_altruism = max_altruism
        self.final_altruism = final_altruism
        self.reward_averaging_weights = np.ones(num_regions) / num_regions
        ...
        self.observation_space = recursive_obs_dict_to_spaces_dict(self.reset())  # This will be set via the env_wrapper (in utils)
        ...
        if self.negotiation_on:
            self.stage = 0
            self.num_negotiation_stages = 3  # 0: rcc simulation, 1: ccf_submission, 2: ccf_adjustment, 3: ccf_mechanism
            self.episode_length += (
                    self.dice_constant["xN"] * self.num_negotiation_stages
            )

            self.actions_nvec += self.ccf.get_actions_nvec()
    ...
    def reset(self):
        if self.negotiation_on and self.ccf.verbosity_level > 0: 
        ...
        if do_profile:
            # profile.print_stats()
            profile.dump_stats("/tmp/rice.lprof")
            # later call python -m line_profiler /tmp/rice.lprof to get the printout
        ...
        self.ccf.reset(self.negotiation_on)
        #        print("!! t", self.timestep)
        #        print("!! c", ccf_parameters.astype("int").reshape((-1,)))
    ...
    def step(self, actions=None):
        ...
        if self.negotiation_on:
            ...
            if self.stage > 0:
                return self.ccf.step(self.stage, actions)

        obs, rew, done, info = self.climate_and_economy_simulation_step(actions)
        self.altruism = max(min(self.altruism, self.max_altruism), self.initial_altruism)
        if self.altruism > 0:
            # replace all regions' reward by a convex combination of own and weighted average of rewards:
            avg_reward = np.sum([
                self.reward_averaging_weights[region_id] * rew[region_id]
                for region_id in range(self.num_regions)
            ]) / self.reward_averaging_weights.sum()
            rew = {
                region_id: self.altruism * avg_reward + (1 - self.altruism) * rew[region_id]
                for region_id in range(self.num_regions)
            }
    ...
    def generate_observation(self):
        ...
        if self.negotiation_on:
            ...
            public_features += self.ccf.get_obs_features()
        ...
        # Combine the features and action mask dictionaries.
        obs = {}
        for region_id in range(self.num_regions):
            obs[region_id] = {
                "obs": features_dict[region_id],
                "action_mask": action_mask_dict[region_id],
                "stage": self.stage,
            }
    ...
    def generate_action_mask(self):
        ...
        """
        return self.ccf.get_action_mask()
